  ...
  <div class="space-y-6">
    ...
    <div class="card overflow-hidden">
      <div class="p-5 border-b border-gray-100">
        ...
        <span
          v-else-if="patient"
          class="font-bold text-lg text-primary-700"
        >
      ...
      >
        <table class="w-full">
          ...
          <tbody>
            ...
            >
              ...
              <td class="p-4">
                <span class="font-semibold text-primary-700">
                  {{ requestCount(currentPatient.patients_id) }}
                </span>
              </td>

              <td class="p-4">
                <StatusBadge
                  v-if="latestStatus(currentPatient.patients_id)"
                  :status="latestStatus(currentPatient.patients_id)!"
                />
            ...
            >
              <td class="p-4 font-mono text-primary-700 font-medium text-sm">
                {{ req.request_id }}
              </td>
              ...
              <td class="p-4 text-sm text-gray-700">
                {{
                  req.TestResult
                    ?.map((r: any) => r.TestType?.test_name)
                    .filter(Boolean)
                    .join(', ') || '—'
                }}
              </td>
  ...
  try {
    ...
    if (cachedProfile) {
      ...
      const fetchedPatient =
        await patientListings.getCurrentPatient()
...
const stats = computed(() => ({
  ...
  pending: requests.value.filter(
    r => r.status === 'Pending'
  ).length,

  completed: requests.value.filter(
